...
const ICONS = {
  ...
  maize: `<svg viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg">
    <rect x="3" y="3" width="26" height="26" rx="3" fill="#111" stroke="#72d46d" stroke-width="1.3"/>
    <path d="M7 8 H25 M7 13 H19 M13 13 V23 M18 8 V18 M23 8 V23 M7 18 H13 M18 18 H23" fill="none" stroke="#72d46d" stroke-width="2"/>
    <rect x="20" y="20" width="5" height="5" fill="#398ee8" stroke="#c8efff" stroke-width=".7"/>
  </svg>`
};
  ...
  sp.stars =
    Array.from(
      ...
      () => ({
        ...
        hue:
          ["255,255,255", "160,210,255", "225,180,255", "160,255,232"][
...
function drawStar(
  ...
  const alpha = .2 + .75 * (.5 + .5 * Math.sin(t * s.twinkleSpeed + s.phase));
  const radius = s.size * (.45 + .35 * (.5 + .5 * Math.sin(t * s.twinkleSpeed * 1.8 + s.phase)));
...
function spaceFrame(t) {
  ...
  spaceGradient.addColorStop(0, "#030817");
  spaceGradient.addColorStop(.44, "#070b27");
  spaceGradient.addColorStop(1, "#12061f");
...
/* ============================================================
   CYBERPUNK SYNTHWAVE WALLPAPER
   ============================================================ */

function cyberResize() {
  const canvas = $("matrixCanvas");

  if (!canvas) return;

  cyber.canvas = canvas;
  cyber.ctx = canvas.getContext("2d");
  cyber.w = canvas.width = window.innerWidth;
  cyber.h = canvas.height = window.innerHeight;
  cyber.stars = Array.from({ length: Math.max(80, Math.round(cyber.w * cyber.h / 8500)) }, () => ({
    x: Math.random() * cyber.w,
    y: Math.random() * cyber.h * .58,
    size: Math.random() * 1.6 + .35,
    phase: Math.random() * Math.PI * 2,
    hue: Math.random() > .72 ? 315 : 188
  }));
}


function cyberFrame(timestamp) {
  const { ctx, w, h, stars } = cyber;

  if (!ctx || !w || !h) return;

  const t = timestamp * .001;
  const horizon = h * .57;
  const glowX = w * (.58 + Math.sin(t * .08) * .1);
  const sky = ctx.createLinearGradient(0, 0, 0, h);
  sky.addColorStop(0, "#090516");
  sky.addColorStop(.42, "#281143");
  sky.addColorStop(.67, "#9b1a72");
  sky.addColorStop(1, "#090d21");
  ctx.fillStyle = sky;
  ctx.fillRect(0, 0, w, h);

  const bloom = ctx.createRadialGradient(glowX, horizon - h * .12, 0, glowX, horizon - h * .12, w * .56);
  bloom.addColorStop(0, "rgba(255,154,78,.35)");
  bloom.addColorStop(.25, "rgba(236,72,153,.16)");
  bloom.addColorStop(1, "rgba(0,0,0,0)");
  ctx.fillStyle = bloom;
  ctx.fillRect(0, 0, w, h);

  stars.forEach((star) => {
    const alpha = .32 + Math.sin(t * 1.7 + star.phase) * .22;
    ctx.fillStyle = `hsla(${star.hue}, 100%, 83%, ${alpha})`;
    ctx.fillRect(star.x, star.y, star.size, star.size);
  });

  const sunY = horizon - h * .12;
  const sunR = Math.min(w, h) * .105;
  const sun = ctx.createRadialGradient(glowX, sunY, 0, glowX, sunY, sunR);
  sun.addColorStop(0, "#fff4c7");
  sun.addColorStop(.42, "#ffb35c");
  sun.addColorStop(1, "#f43f9a");
  ctx.fillStyle = sun;
  ctx.beginPath();
  ctx.arc(glowX, sunY, sunR, 0, Math.PI * 2);
  ctx.fill();
  ctx.strokeStyle = "rgba(83,7,81,.38)";
  ctx.lineWidth = 2;
  for (let i = -4; i <= 4; i += 1) {
    const y = sunY + i * sunR * .19;
    ctx.beginPath();
    ctx.moveTo(glowX - sunR, y);
    ctx.lineTo(glowX + sunR, y);
    ctx.stroke();
  }

  ctx.fillStyle = "rgba(4,4,20,.62)";
  ctx.fillRect(0, horizon, w, h - horizon);
  ctx.save();
  ctx.beginPath();
  ctx.rect(0, horizon, w, h - horizon);
  ctx.clip();
  ctx.strokeStyle = "rgba(28,233,255,.5)";
  ctx.lineWidth = 1;
  ...
  for (let x = -14; x <= 14; x += 1) {
    ...
    ctx.lineTo(w / 2 + x * w * .115, h);
    ctx.stroke();
  }
  ctx.restore();

  wallpaperRAF = requestAnimationFrame(cyberFrame);
}


function startCyberpunkWallpaper() {
  cyberResize();
  wallpaperRAF = requestAnimationFrame(cyberFrame);
}
...
/* ============================================================
   SECRET GLITCH WALLPAPER
   ============================================================ */

function secretGlitchResize() {
  const canvas = $("matrixCanvas");

  if (!canvas) return;

  secretGlitch.canvas = canvas;
  secretGlitch.ctx = canvas.getContext("2d");
  secretGlitch.w = canvas.width = window.innerWidth;
  secretGlitch.h = canvas.height = window.innerHeight;
  secretGlitch.fragments = Array.from({ length: 42 }, () => ({
    x: Math.random() * secretGlitch.w,
    y: Math.random() * secretGlitch.h,
    w: 20 + Math.random() * secretGlitch.w * .35,
    h: 1 + Math.random() * 15,
    phase: Math.random() * Math.PI * 2
  }));
}
...
function secretGlitchFrame(timestamp) {
  const { ctx, w, h, fragments } = secretGlitch;

  if (!ctx || !w || !h || !fragments) return;

  const t = timestamp * .001;
  ctx.fillStyle = "#05030a";
  ctx.fillRect(0, 0, w, h);

  const bands = ["0,245,255", "255,38,58", "255,228,47"];
  for (let row = 0; row < 16; row += 1) {
    const y = row / 16 * h;
    const offset = Math.sin(t * 8 + row * 4) * (row % 3 === 0 ? 38 : 9);
    ctx.fillStyle = `rgba(${bands[row % 3]},${.025 + (row % 4) * .01})`;
    ctx.fillRect(offset, y, w, h / 16 + 1);
  }

  fragments.forEach((fragment, index) => {
    const energy = .18 + .5 * (.5 + .5 * Math.sin(t * (4 + index % 5) + fragment.phase));
    const jump = Math.sin(t * 18 + index * 13) * (index % 4 === 0 ? 52 : 13);
    ctx.fillStyle = `rgba(${bands[index % 3]},${energy})`;
    ctx.fillRect(fragment.x + jump, fragment.y, fragment.w, fragment.h);
  });

  ctx.fillStyle = "rgba(255,255,255,.08)";
  for (let y = 0; y < h; y += 3) ctx.fillRect(0, y, w, 1);

  const markerX = w * (.46 + Math.sin(t * 2.2) * .08);
  ctx.font = "bold 13px monospace";
  ctx.fillStyle = "rgba(0,245,255,.55)";
  ctx.fillText("// SIGNAL NOT STABLE", markerX, h * .32);
  ctx.fillStyle = "rgba(255,38,58,.48)";
  ctx.fillText("// SIGNAL NOT STABLE", markerX + 5, h * .32 + 2);
  ctx.fillStyle = "rgba(255,228,47,.33)";
  ctx.fillText("// SIGNAL NOT STABLE", markerX - 4, h * .32 - 2);

  wallpaperRAF = requestAnimationFrame(secretGlitchFrame);
}


function startSecretGlitchWallpaper() {
  secretGlitchResize();
  wallpaperRAF = requestAnimationFrame(secretGlitchFrame);
}
...
function setWallpaper(mode) {
  ...
  if (mode === "secret" && !secretUnlocked) {
    const accessCode = window.prompt("SECRET THEME // ENTER ACCESS CODE");

    if (accessCode === null) return;

    if (accessCode !== "Ritam200+JEE") {
      showMessageBox({
        title: "Access Denied",
        icon: ICONS.error,
        text: "SECRET THEME ACCESS CODE REJECTED."
      });
      return;
    }
  ...
  document.body.classList.toggle("eco-tech", mode === "eco");
  document.body.classList.toggle("secret-theme", mode === "secret");
  ...
  if (mode === "solid") {
    ...
    // The static Eco-Friendly Aero wallpaper is supplied by CSS.
    desktop.style.background = "";
...
function buildTechCalendar() {
  const grid = $("techCalendar");
  ...
  for (let day = 1; day <= days; day += 1) {
    ...
    if (day === 24) cell.classList.add("event");
...
function updateTechDashboard() {
  ...
  const mainTime = $("techMainTime");
  const statusTime = $("techStatusTime");
  const dateLabel = $("techDate");

  if (mainTime) mainTime.textContent = time;
  if (statusTime) statusTime.textContent = time;
  if (dateLabel) dateLabel.textContent = date;

  const phase = now.getSeconds() / 60 * Math.PI * 2;
  const cpu = Math.round(24 + Math.sin(phase) * 8 + Math.random() * 3);
  const ram = Math.round(62 + Math.cos(phase * .75) * 4);
  const down = (42.5 + Math.sin(phase * 1.7) * 5).toFixed(1);
  const up = (12.1 + Math.cos(phase * 1.4) * 2).toFixed(1);

  const cpuText = $("techCpu");
  const ramText = $("techRam");
  const cpuBar = $("techCpuBar");
  const ramBar = $("techRamBar");
  const downText = $("techDown");
  const upText = $("techUp");
  ...
  if (downText) downText.textContent = down;
  if (upText) upText.textContent = up;
}
...
function initTechDashboard() {
  buildTechCalendar();
  updateTechDashboard();
  setInterval(updateTechDashboard, 1000);
...
function toggleTechPanel(panelId) {
  const panel = $(panelId);
  ...
  document.querySelectorAll(".tech-popout").forEach((item) => {
    item.classList.remove("active");
  });

  document.querySelectorAll(".tray-app").forEach((item) => {
    item.classList.remove("active");
  });
